...
public class StudentDatabase {
    ...
    public static List<Course> loadCoursesFromExcel() {
            ...
            for (int rowIndex = 1; rowIndex <= sheet.getLastRowNum(); rowIndex++) { // Start from row 1 (skip header)
                ...
                if (row != null) {
                    try {
                        ...
                        Course course = new Course(courseCode, courseName, subjectName, sectionNumber, teacherName, capacity, lectureTime, finalExamDateTime, location);
                        courses.add(course);
                    } catch (Exception e) {
                        System.err.println("Error reading row " + rowIndex + ": " + e.getMessage());
                        e.printStackTrace(); // Print the full stack trace for debugging
    ...
    public static List<Course> getCoursesForStudent(String studentId) {
        ...
        if (student != null) {
            String subjectsRegistered = student.getSubjectsRegistered();
            if (subjectsRegistered != null && !subjectsRegistered.isEmpty()) {
                String[] subjectCodes = subjectsRegistered.split(","); // Assuming comma-separated subject codes
                List<Course> allCourses = loadCoursesFromExcel();

                for (String subjectCode : subjectCodes) {
                    for (Course course : allCourses) {
                        if (course.getSubjectName().trim().equalsIgnoreCase(subjectCode.trim())) {
